...
if [ $service = "DELETE" ] || [ $service = "RESTORE" ]; then
 	java -classpath bin peer$peer $service $file
fi
if [ $service = "RECLAIM" ]; then
	java -classpath bin peer$peer RECLAIM $space
fi
if [ $service = "BACKUP" ]; then
	java -classpath bin peer$peer BACKUP $file $replic
fi
if [ $service = "STATE" ]; then
	java -classpath bin peer$peer STATE
fi
